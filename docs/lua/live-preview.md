...
The right pane uses the production semantic palette, padded inset panel, and control-aware node layout. Sample inputs and fields are editable. Running the preview advances isolated preview ticks and state; Reset recreates the node from its defaults.
