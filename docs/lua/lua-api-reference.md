...
Adds a `number`, `text`, `boolean`, `choice`, `color`, `direction`, or `item` field. Options include `default`, numeric `min`/`max`, and `choices` for choice fields. Errors on invalid ranges or an empty choice list. Example: `node:field("mode", "choice", { choices = { "a", "b" }, default = "a" })`.
