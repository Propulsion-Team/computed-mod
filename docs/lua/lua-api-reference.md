...
Adds a `number`, `text`, `boolean`, `choice`, `color`, `direction`, or `item` field. Every field is rendered as an editable control inside each node instance. Options include `default`, an optional display `label`, numeric `min`/`max`, a positive numeric `step`, and `choices` for choice fields. Number fields default to `control = "value"`; `control = "slider"` requires finite `min` and `max` values with `max > min`. Text uses a value box, booleans use toggles, choice and direction fields use dropdowns, colors use ARGB hexadecimal controls, and items open the searchable item picker. Errors on invalid defaults, ranges, steps, controls, or empty choice lists. Example: `node:field("gain", "number", { default = 1, min = 0, max = 4, control = "slider", step = 0.1, label = "Gain" })`.
